...
/**
 * This action can be used in order to add a new Real value to a given variable.
 * 
 * @author <a href="mailto:laurent.goubet@obeo.fr">Laurent Goubet</a>
 */
public final class NewFloatValueAction extends Action {
	...
	/**
	 * Instantiates the "new value > Real" action given the variable to which we are adding a value.
	 * 
	 * @param viewer
	 *            The variable viewer.
	 * @param variable
	 ...
	 */
	public NewFloatValueAction(TreeViewer variableViewer, Variable variable) {
		super(InterpreterMessages.getString("interpreter.action.newvalue.float.name")); //$NON-NLS-1$
	...
	public void run() {
		InputDialog dialog = new InputDialog(PlatformUI.getWorkbench().getDisplay().getActiveShell(),
				InterpreterMessages.getString("interpreter.action.newvalue.float.popup.title"), //$NON-NLS-1$
				InterpreterMessages.getString("interpreter.action.newvalue.float.popup.message") + ':', "", //$NON-NLS-1$ //$NON-NLS-2$
				new RealValueValidator());
		int result = dialog.open();
		if (result == Window.OK) {
			Object value = variable.getValue();
			Object addedValue = Double.valueOf(dialog.getValue());
		...
		public String isValid(String newText) {
			...
			if (newText == null || newText.length() == 0) {
				errorMessage = InterpreterMessages
						.getString("interpreter.action.newvalue.float.popup.error.novalue"); //$NON-NLS-1$
			} else if (!isReal(newText)) {
				errorMessage = InterpreterMessages.getString(
						"interpreter.action.newvalue.float.popup.error.invalid", newText); //$NON-NLS-1$
			}
			return errorMessage;
		}
