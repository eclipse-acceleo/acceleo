...
public final class RenameVariableAction extends Action {
	 ...
	 * 
	 * @return The currently selected Variable, or the first Variable of the current selection.
	 *         <code>null</code> if none.
	 */
	...
	private void createTextEditor(final Variable variable) {
		...
		textEditor.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				...
				// characters.
				Point parentSize = textEditorParent.getSize();
				textEditor.setBounds(2, inset, Math.min(textSize.x, parentSize.x - 4), parentSize.y - 2
						* inset);
