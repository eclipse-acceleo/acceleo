...
/**
 * This listener will allow us to be aware of contribution changes against the dynamic modules extension
 * point.
 * 
 * @author <a href="mailto:stephane.begaudeau@obeo.fr">Stephane Begaudeau</a>
 */
public class DynamicModulesRegistryListener implements IRegistryEventListener {
	/** Name of the extension point to parse for module locations. */
	public static final String DYNAMIC_MODULES_EXTENSION_POINT = "org.eclipse.acceleo.engine.dynamic.modules"; //$NON-NLS-1$

	/** Name of the extension point's modules tag "path" atribute. */
	private static final String DYNAMIC_MODULES_ATTRIBUTE_PATH = "path"; //$NON-NLS-1$

	/** Name of the extension point's "modules" tag. */
	private static final String DYNAMIC_MODULES_TAG_MODULES = "modules"; //$NON-NLS-1$

	/** Name of the extension point's "generator" tag. */
	private static final String DYNAMIC_MODULES_TAG_GENERATOR = "generator"; //$NON-NLS-1$
	...
	public void parseInitialContributions() {
		...
		// Dynamic templates
		for (IExtension extension : registry.getExtensionPoint(DYNAMIC_MODULES_EXTENSION_POINT)
				.getExtensions()) {
			parseExtension(extension);
		}
	}
	...
	public void removed(IExtension[] extensions) {
		/*
		 * Extensions will be removed on the fly by AcceleoDynamicModulesEclipseUtil when trying to access
		 * uninstalled bundles
		 */
	}
	...
	private void parseExtension(IExtension extension) {
		...
		for (IConfigurationElement elem : configElements) {
			...
			} else if (DYNAMIC_MODULES_TAG_GENERATOR.equals(elem.getName())) {
