...
/**
 * A template completion proposal with dynamic variables. It creates a Let-ElseLet block with a rule for each
 * selected type of the ProposalsBrowser view. The types are well ordered.
 * 
 * @author <a href="mailto:jonathan.musset@obeo.fr">Jonathan Musset</a>
 */
public class NewLetElseLetForeachType implements IAcceleoPatternProposal {
	...
	public String createTemplateProposal(List<EClass> types, String indentTab) {
		...
		if (types.size() > 0) {
			...
			for (int i = 1; i < types.size(); i++) {
				...
				buffer.append(indentTab);
				buffer.append("[elselet ${e} : "); //$NON-NLS-1$
