 ...
 * </copyright>
 *
 * $Id: AbstractEList.java,v 1.1 2011/03/08 13:55:40 sbegaudeau Exp $
 */
...
public abstract class AbstractEList<E> extends AbstractList<E> implements EList<E> {
	...
	/**
	 * Returns whether <code>equals</code> rather than <code>==</code> should be used to compare members. The
	 * default is to return <code>true</code> but clients can optimize performance by returning
	 * <code>false</code>. The performance difference is highly significant.
	 * 
	...
	protected boolean equalObjects(Object firstObject, Object secondObject) {
		return useEquals() && firstObject != null ? firstObject.equals(secondObject)
				: firstObject == secondObject;
	}

	/**
	 * Returns whether <code>null</code> is a valid object for the list. The default is to return
	 * <code>true</code>, but clients can override this to exclude <code>null</code>.
	 * 
	...
	/**
	 * Returns whether objects are constrained to appear at most once in the list. The default is to return
	 * <code>false</code>, but clients can override this to ensure uniqueness of contents. The performance
	 * impact is significant: operations such as <code>add</code> are O(n) as a result requiring uniqueness.
	 * 
	...
	/**
	 * Validates a new content object and returns the validated object. This implementation checks for null,
	 * if {@link #canContainNull necessary} and returns the argument object. Clients may throw additional
	 * types of runtime exceptions in order to handle constraint violations.
	 * 
	 * @param index
	...
	protected E validate(int index, E object) {
		if (!canContainNull() && object == null) {
			throw new IllegalArgumentException("The 'no null' constraint is violated"); //$NON-NLS-1$
		}
	...
	/**
	 * Resolves the object at the index and returns the result. This implementation simply returns the
	 * <code>object</code>; clients can use this to transform objects as they are fetched.
	 * 
	 * @param index
	...
	/**
	 * Called to indicate that the data storage has been set. This implementation does nothing; clients can
	 * use this to monitor settings to the data storage.
	 * 
	 * @param index
	...
	/**
	 * Called to indicate that an object has been added to the data storage. This implementation does nothing;
	...
	/**
	 * Called to indicate that an object has been removed from the data storage. This implementation does
	 * nothing; clients can use this to monitor removals from the data storage.
	 * 
	 * @param index
	...
	/**
	 * Called to indicate that the data storage has been cleared. This implementation calls {@link #didRemove
	 * didRemove} for each object; clients can use this to monitor clearing of the data storage.
	 * 
	 * @param size
	...
	/**
	 * Called to indicate that an object has been moved in the data storage. This implementation does nothing;
	...
	/**
	 * Called to indicate that the data storage has been changed. This implementation does nothing; clients
	 * can use this to monitor change in the data storage.
	 */
	...
	/**
	 * An IndexOutOfBoundsException that constructs a message from the argument data. Having this avoids
	 * having the byte code that computes the message repeated/inlined at the creation site.
	 */
	protected static class BasicIndexOutOfBoundsException extends IndexOutOfBoundsException {
		...
		public BasicIndexOutOfBoundsException(int index, int size) {
			super("index=" + index + ", size=" + size); //$NON-NLS-1$ //$NON-NLS-2$
		}
	}
	...
	/**
	 * Returns the object at the index without {@link #resolve resolving} it and without range checking the
	 * index.
	 * 
	 * @param index
	 *            the position in question.
	...
	/**
	 * Sets the object at the index and returns the old object at the index. This implementation delegates to
	 * {@link #setUnique setUnique} after range checking and after {@link #isUnique uniqueness} checking.
	 * 
	 * @param index
	 *            the position in question.
	 * @param object
	...
	/**
	 * Sets the object at the index and returns the old object at the index; it does no ranging checking or
	 * uniqueness checking. This implementation delegates to {@link #assign assign}, {@link #didSet didSet},
	...
	/**
	 * Adds the object at the end of the list and returns whether the object was added; if {@link #isUnique
	 * uniqueness} is required, duplicates will be ignored and <code>false</code> will be returned. This
	 * implementation delegates to {@link #addUnique(Object) addUnique(E)} after uniqueness checking.
	 * 
	 * @param object
	...
	/**
	 * Adds the object at the end of the list; it does no uniqueness checking. This implementation delegates
	 * to {@link #assign assign}, {@link #didAdd didAdd}, and {@link #didChange didChange}. after uniqueness
	 * checking.
	 * 
	 * @param object
	 *            the object to be added.
	...
	/**
	 * Adds the object at the given index in the list. If {@link #isUnique uniqueness} is required, duplicates
	 * will be ignored. This implementation delegates to {@link #addUnique(int, Object) addUnique(int, E)}
	 * after uniqueness checking.
	 * 
	 * @param object
	 *            the object to be added.
	 * @exception IllegalArgumentException
	 *                if {@link #isUnique uniqueness} is required, and the object is a duplicate.
	 ...
	 * This implementation delegates to {@link #assign assign}, {@link #didAdd didAdd}, and {@link #didChange
	 * didChange}.
	 * 
	 * @param object
	 *            the object to be added.
	...
	/**
	 * Adds each object of the collection to the end of the list. If {@link #isUnique uniqueness} is required,
	 * duplicates will be {@link #getNonDuplicates removed} from the collection, which could even result in an
	 * empty collection. This implementation delegates to {@link #addAllUnique(Collection)
	 * addAllUnique(Collection)} after uniqueness checking.
	 * 
	 * @param collection
	...
	/**
	 * Adds each object of the collection to the end of the list; it does no uniqueness checking. This
	 * implementation delegates to {@link #assign assign}, {@link #didAdd didAdd}, and {@link #didChange
	 * didChange}.
	 * 
	 * @param collection
	 *            the collection of objects to be added.
	...
	/**
	 * Adds each object of the collection at each successive index in the list and returns whether any objects
	 * were added. If {@link #isUnique uniqueness} is required, duplicates will be {@link #getNonDuplicates
	 * removed} from the collection, which could even result in an empty collection. This implementation
	 * delegates to {@link #addAllUnique(int, Collection) addAllUnique(int, Collection)} after uniqueness
	 * checking.
	 * 
	 * @param index
	...
	/**
	 * Adds each object of the collection at each successive index in the list and returns whether any objects
	 * were added; it does no ranging checking or uniqueness checking. This implementation delegates to
	 * {@link #assign assign}, {@link #didAdd didAdd}, and {@link #didChange didChange}.
	 * 
	 * @param index
	 *            the index at which to add.
	 * @param collection
	...
	/**
	 * Adds each object from start to end of the array at the index of list and returns whether any objects
	 * were added; it does no ranging checking or uniqueness checking. This implementation delegates to
	 * {@link #assign assign}, {@link #didAdd didAdd}, and {@link #didChange didChange}.
	 * 
	 * @param objects
	...
	/**
	 * Adds each object from start to end of the array at each successive index in the list and returns
	 * whether any objects were added; it does no ranging checking or uniqueness checking. This implementation
	 * delegates to {@link #assign assign}, {@link #didAdd didAdd}, and {@link #didChange didChange}.
	 * 
	 * @param index
	 *            the index at which to add.
	 * @param objects
	 ...
	 * This implementation uses {@link #indexOf indexOf} to find the object and delegates to
	 * {@link #remove(int) remove(int)} in the case that it finds the object.
	 * 
	 * @param object
	...
	/**
	 * Removes each object of the collection from the list and returns whether any object was actually
	...
	/**
	 * Removes the object at the index from the list and returns it. This implementation delegates to
	 * {@link #didRemove didRemove} and {@link #didChange didChange}.
	 * 
	 * @param index
	...
	/**
	 * Removes from the list each object not contained by the collection and returns whether any object was
	 * actually removed. This delegates to {@link #remove(int) remove(int)} in the case that it finds an
	 * object that isn't retained.
	 * 
	 * @param collection
	...
	/**
	 * Moves the object to the index of the list. This implementation uses {@link #indexOf} of find the object
	 ...
	 * @exception IndexOutOfBoundsException
	 *                if the index isn't within the size range or the object isn't contained by the list.
	 */
	...
	/**
	 * Moves the object at the source index of the list to the target index of the list and returns the moved
	 * object. This implementation delegates to {@link #assign assign}, {@link #didMove didMove}, and
	...
	/**
	 * Returns an iterator. This implementation allocates a {@link AbstractEList.EIterator}.
	 * 
	 * @return an iterator.
	...
	protected class EIterator<E1> implements Iterator<E1> {
		...
		/**
		 * Returns the next object and advances the iterator. This implementation delegates to {@link #doNext
		 * doNext}.
		 * 
		...
		/**
		 * Returns the next object and advances the iterator. This implementation delegates to
		...
		/**
		 * Removes the last object returned by {@link #next()} from the list, it's an optional operation. This
		 * implementation can also function in a list iterator to act upon on the object returned by calling
		 * <code>previous</code>.
		 * 
		 * @exception IllegalStateException
		 *                if <code>next</code> has not yet been called, or <code>remove</code> has already
	...
	/**
	 * Returns a read-only iterator that does not {@link #resolve resolve} objects. This implementation
	 * allocates a {@link NonResolvingEIterator}.
	 * 
	...
	protected class NonResolvingEIterator<E1> extends EIterator<E1> {
		/**
		 * Returns the next object and advances the iterator. This implementation accesses the data storage
		 * directly.
		 * 
		 * @return the next object.
		 * @exception NoSuchElementException
	...
	/**
	 * Returns a list iterator. This implementation allocates a {@link AbstractEList.EListIterator}.
	 * 
	...
	/**
	 * Returns a list iterator advanced to the given index. This implementation allocates a
	 * {@link AbstractEList.EListIterator}.
	 * 
	 * @param index
	 *            the starting index.
	...
	/**
	 * Returns a read-only list iterator that does not {@link #resolve resolve} objects. This implementation
	 * allocates a {@link NonResolvingEListIterator}.
	 * 
	...
	/**
	 * Returns a read-only list iterator advanced to the given index that does not {@link #resolve resolve}
	 * objects. This implementation allocates a {@link NonResolvingEListIterator}.
	 * 
	 * @param index
	 *            the starting index.
	...
	protected class NonResolvingEListIterator<E1> extends EListIterator<E1> {
		...
		/**
		 * Returns the previous object and advances the iterator. This implementation accesses the data
		 * storage directly.
		 * 
		 * @return the previous object.
		 * @exception NoSuchElementException
	...
	/**
	 * Returns an <b>unsafe</b> list that provides a {@link #resolve non-resolving} view of the underlying
	 * data storage.
	 * 
	...
	protected Collection<E> getDuplicates(Collection<?> collection) {
		if (collection.isEmpty()) {
			...
			Collection<E> filteredResult = useEquals() ? new BasicEList<E>(collection.size())
					: new BasicEList.FastCompare<E>(collection.size());
			for (E object : this) {
	...
	protected Collection<E> getNonDuplicates(Collection<? extends E> collection) {
		Collection<E> result = useEquals() ? new UniqueEList<E>(collection.size())
				: new UniqueEList.FastCompare<E>(collection.size());
