...
//CHECKSTYLE:OFF
/**
 * A highly extensible abstract list implementation {@link #data() logically backed by an array} that is
 * <b>never<b> modified.
 * 
 * @since 3.1
 */
public abstract class ArrayDelegatingEList<E> extends AbstractEList<E> implements RandomAccess, Cloneable, Serializable {
	...
	/**
	 * Returns new allocated data storage. Clients may override this to create typed storage. The cost of type
	 * checking via a typed array is negligible.
	 * 
	...
	/**
	 * Returns the position of the last occurrence of the object in the list. This implementation uses either
	 * <code>equals</code> or <code>"=="</code> depending on {@link #useEquals useEquals}.
	 * 
	 * @param object
	 *            the object in question.
	...
	/**
	 * Returns an array containing all the objects in sequence. Clients may override {@link #newData newData}
	 ...
	 * 
	 * @param array
	 *            the array that will be filled and returned, if it's big enough; otherwise, a suitably large
	 *            array of the same type will be allocated and used instead.
	 * @return an array containing all the objects in sequence.
	 * @see #newData
	 */
	@Override
	...
	/**
	 * Returns direct <b>unsafe</b> access to the underlying data storage. Clients may <b>not</b> modify this
	 * array's elements but <b>may</b> assume that the array remains valid even as the list is modified. The
	 * array's length is exactly the same as the list's {@link #size() size}; <code>null</code> is used to
	...
	/**
	 * Updates directly and <b>unsafely</b> the underlying data storage. Clients <b>must</b> be aware that
	 * this subverts all callbacks and hence possibly the integrity of the list. The list implementation
	 * itself calls this method whenever a modification of the list requires a new backing array.
	 * 
	 * @param data
	 *            the new underlying data storage.
	 */
	...
	/**
	 * Returns the object at the index. This implementation delegates to {@link #resolve resolve} so that
	 * clients may transform the fetched object.
	 * 
	 * @param index
	...
	public boolean retainAll(Collection<?> collection) {
		...
		int size = data == null ? 0 : data.length;
		for (int i = size; --i >= 0;) {
			if (data != null && !collection.contains(data[i])) {
				remove(i);
				modified = true;
			}
		}
	...
	/**
	 * Clears the list of all objects. This implementation discards the data storage without modifying it and
	 * delegates to {@link #didClear didClear} and {@link #didChange didChange}.
	 */
	@Override
	public void clear() {
		++modCount;
	...
	public Object clone() {
		try {
			@SuppressWarnings("unchecked")
			ArrayDelegatingEList<E> clone = (ArrayDelegatingEList<E>)super.clone();
			Object[] data = data();
			int size = data == null ? 0 : data.length;
			if (size > 0) {
	...
	/**
	 * Returns an iterator. This implementation allocates a {@link ArrayDelegatingEList.EIterator}.
	 * 
	 * @return an iterator.
	...
	/**
	 * Returns a list iterator. This implementation allocates a {@link ArrayDelegatingEList.EListIterator}.
	 * 
	 ...
	 * {@link ArrayDelegatingEList.EListIterator}.
	 * 
	 * @param index
	 *            the starting index.
