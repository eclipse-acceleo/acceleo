...
/**
 * Wrap the selected statements in a {@link ForStatement}.
 * 
 * @author <a href="mailto:yvan.lussaud@obeo.fr">Yvan Lussaud</a>
 */
public class WrapInForCommand extends AbstractDocumentRangeCommand {
	...
	public WorkspaceEdit exec(AcceleoTextDocument document, Range range) {
		...
		if (range.getStart().getCharacter() == 0 && range.getEnd().getCharacter() == 0) {
			...
			if (isSameBlock(document, startIndex, endIndex)) {
				...
				binding.setName(AcceleoCodeTemplates.DEFAULT_NEW_BINDING_VARIABLE_NAME);
				...
				binding.setInitExpression(expression);
				statement.setBinding(binding);
