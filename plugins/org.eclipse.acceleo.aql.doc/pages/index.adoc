...
If you want to get a bit further, you can have a look at the <<launching_an-acceleo_generation,Launching an Acceleo generation>> section. If you prefere to practice by yourself you can try the https://github.com/eclipse-acceleo/acceleo/blob/master/examples/maven-textual-game/tutorial.adoc[Maven tutorial] it covers basic template writing and best practices for Acceleo.
...
** Only available if the project containing the main module file is an Eclipse plug-in project
** Creates an Eclipse plug-in project alongside your generator project
** Extends the standalone class (the standalone main class is generated if needed)
** Contributes a menu for objects that can be passed to your main template
** Also contributes a menu for files with the extension corresponding to the metamodel (this contribution is disabled by default, the corresponding code is commented in plugin.xml)

[NOTE]
====
The generated code is provided as a sample and must be manually adapted (for example, the menu names).
====
...
You can launch a generation in full Maven if needed. It is covered in the previous section, but if you want a pure Maven example you can have a look at this [folder]. The Maven example also comes with a https://github.com/eclipse-acceleo/acceleo/blob/master/examples/maven-textual-game/tutorial.adoc[tutorial] that explain the project structure.
...
The following https://github.com/eclipse-acceleo/acceleo/tree/master/tests/maven/org.eclipse.acceleo.aql.maven.tests[test project] can be used as a simple example. For a more complete example with a custom metamodel you can check the https://github.com/eclipse-acceleo/acceleo/blob/master/examples/maven-textual-game/tutorial.adoc[textual game tutorial].
...
The migration tool doesn't cover all migration cases and you will need to manually fix remaining problems. The tool will migrate modules and services Java classes. It will also copy other resources. If you have generated Java launcher and/or Eclipse UI plug-in launcher, you will need to <<java-launchers,generate>> them from Acceleo 4.
