...
* A very narrow dependency surface: AQL uses the very central parts of EMF and ANTLR so that we could easily deploy AQL outside of Eclipse in a server or a standalone scenario.
...
Filtering a collection is generally done using either `->filter(..)` to keep elements of a given type or `->select(..)` to keep elements which are validating a given condition.
...
The variable *self* here is the *Class* named 'Invoice' in the model, as such the query `self` highlight this instance.
...
Returns a set containing the result of applying "lambda" recursively.
...
Converts the given String in a Sequence of Strings representing each character.
...
Substitutes all occurrences of the substring "r" in self by "t" and returns the resulting string. Will return self if it contains no occurrence of the substring r.
...
When a call or a feature access is done on a collection the result is flattened for the first level. For instance a service returning a collection called on a collection will return a collection of elements and not a collection of collection of elements.
...
Furthermore *oclIsUndefined() does not exist in AQL* and should be replaced by a `...  = null` expression. In MTL, expressions such as `anElement.getMyAttribute().oclIsUndefined() or anElement.getMyAttribute().oclIsInvalid()` are quite common. This kind of expression can be safely replaced by `anElement.getMyAttribute() = null`.
...
* Development environment (workspace): In this context the resolution should be divided if you work with projects where each project is responsible for the resolution of its own resources. That will prevent loading multiple times the same resource.
